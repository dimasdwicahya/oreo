...
if(isset($_POST['daftar'])){
	...
	$allowed_ext  = array('jpg', 'jpeg', 'png', 'gif'); // Jenis file yang diperbolehkan untuk diupload
      ...
      $location       = 'lampiran/'.$notiket.'.'.$file_ext; // Menentukan lokasi upload dan menggabungkan dengan judul_seo dan ekstensi file yang diupload
      ...
      if(in_array($file_ext, $allowed_ext) === true) // Pengecekan tipe file yang diperbolehkan
      {
        ...
        {
          echo "<script>alert('Insert data berhasil! Klik ok untuk melanjutkan');location.replace('halaman-lihat-tiket.php')</script>";
        }
          else
          {
